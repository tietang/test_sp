...
$(function() {
	initManageMenu();
	$("#manage_menu a").first().trigger("click");

});
function initManageMenu() {
	$("#manage_menu a").each(function(index) {
		$(this).click(function() {
...
function onDeletePhotoSet(name, idSet) {
	bootbox.confirm(i18n.Dir.Delete.confirm, function(result) {
		if (result) {
...
function getDirNavPage() {
	$.get("/photo/manage/set/nav", function(data) {
