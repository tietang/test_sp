...
function fileQueueError(file, errorCode, message) {
	try {
		...
		addImage("public/swfupload/images/" + imageName);
...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		if (numFilesQueued > 0) {
			this.startResizedUpload(this.getFile(0).ID,
					this.customSettings.thumbnail_width,
					this.customSettings.thumbnail_height,
...
function uploadProgress(file, bytesLoaded) {
	...
	try {
		...
		$('#progress .ui-progressingbar').css('width', percent + '%');
...
function uploadSuccess(file, serverData) {
	try {
		...
		addImage( serverData.name.substr(2));
